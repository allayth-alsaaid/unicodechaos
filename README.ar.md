<div align="center">
  ...
  <h1>Unicode Chaos</h1>
  <p><b>كل حروف العالم في نص واحد.</b> مولّد نصوص بأقصى تنوع يمزج 170 نظام كتابة ورموزاً وأرقاماً وإيموجي — ببذور قابلة لإعادة الإنتاج وبدون أي باك-إند.</p>
  <p>
...
يبني Unicode Chaos نصاً **لأقصى تنوع لا لأي معنى**. حروف من 170 سكريبت تصطدم بالرموز والأرقام والإيموجي عبر محرك عشوائي موزون بضمانات صارمة ضد التكرار.
...
عشوائية النظام (256 بت) + jitter + طابع ميكروثاني + عدّاد
   ...
   البذرة (64 خانة ست عشرية)
        ...
        ▼
فئة ← سكريبت ← حرف   (موزون + ضد تكرار + تعزيز النادر)
        │
        ▼
  نص مرئي (الطول يُحسب بالـgraphemes — الإيموجي = 1)
```

- **سحب بثلاث مراحل:** أوزان الفئات حسب مستوى الفوضى، ثم سكريبت بحجم مداه × تعزيز الندرة × عقوبة التكرار، ثم code point منتظم من مدياته.
- **لا يُخرَج أبداً:** علامات التشكيل والحروف الخفية/التحكم (تفسد العد والعرض).
- **لا تكرار أبداً:** نفس السكريبت مرتين متتاليتين (كل المستويات)، نفس الحرف ضمن آخر وحدتين، نفس الفئة ثلاث مرات — إلا إذا عزلت سكريبت/فئة واحدة عمداً، فتنحني القاعدة للفيزياء.
...
| الفحص | النتيجة |
|---|---|
| التنوع الكامل، قصوى، طول 5000 | **176 / 176** مميز |
| عزل كل سكريبت وحده | نقي 100% بتسميات صحيحة |
| استبعاد كل سكريبت وحده | صفر تسريب |
| عزل/استبعاد كل فئة | نقي / صفر تسريب |
| إيقاف الإيموجي، طول 2000 × 4 مستويات | **صفر تسريب** |
...
js/generator.js     محرك فئة ← سكريبت ← حرف
js/diversity.js     قواعد ضد التكرار + تعزيز النادر
js/unicode.js       تحميل البيانات + فلاتر (تشكيل، خفي، إيقاف إيموجي)
js/weights.js       أوزان الفئات حسب مستوى الفوضى
...
> **ملاحظة:** تم إنشاء هذا المشروع باستخدام الذكاء الاصطناعي.
