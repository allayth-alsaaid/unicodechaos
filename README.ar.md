...
يبني Unicode Chaos نصاً **لعشوائية خالصة لا لأي معنى**. حروف من 170 سكريبت تصطدم بالرموز والأرقام والإيموجي عبر محرك عشوائي منتظم: كل سكريبت وفئة-قائمة باحتمال متساوٍ (≈1/176 لكل منها)، وكل سحبة مستقلة.
...
عشوائية النظام (256 بت) + jitter + طابع ميكروثاني + عدّاد
   ...
   البذرة (64 خانة ست عشرية)
        ...
        ▼
فئة ← سكريبت ← حرف   (pool منتظم مسطّح: 170 سكريبت + 6 قوائم)
        │
        ▼
  نص مرئي (الطول يُحسب بالـgraphemes — الإيموجي = 1)
```

- **سحب بثلاث مراحل:** سحبة منتظمة واحدة من 176 مدخلًا (170 سكريبت + رموز/إيموجي/أرقام/ترقيم/عملات/أخرى)، ثم code point منتظم من السكريبت الفائز — أو عنصر منتظم من القائمة الفائزة. بلا أوزان ولا تعزيز ولا قواعد ضد التكرار — النسب تتبع الصدفة لا خطة.
...
| الفحص | النتيجة |
|---|---|
| سحب منتظم مسطّح، طول 17600 | 176/176 مدخلًا ≈ 1/176 لكل منها (أدنى 71، أقصى 139) · إيموجي ≈ 0.65% |
